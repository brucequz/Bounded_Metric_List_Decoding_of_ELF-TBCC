...
MessageInformation LowRateListDecoder::genieAided_LowRateDecoding_MaxListsize_Collect(std::vector<float> receivedMessage, std::vector<int> punctured_indices, std::vector<int> transmittedCodeword, std::vector<float> sampling_points, std::vector<float> listsize_collect_sample_points){
  ...
  float running_min_highrate_to_tx = 1000.0;
	std::vector<float> lower_envelop(100, -1.0f);
  size_t last_end = 0;
    ...
    float highrate_to_projected_rx = utils::euclidean_distance(codeword, receivedMessage, punctured_indices);
		...
		// update running minimum of lower envelop
    ...
    for (size_t id = last_end; id < sampling_points.size(); id++) {
      if (highrate_to_projected_rx >= sampling_points[id]) {
        lower_envelop[id] = running_min_highrate_to_tx;
      } else {
        last_end = id;
        break;
      }
		...
		if(path[0] == path[lowrate_pathLength - 1] && crc::crc_check(message, crcLength, crc) && numPathsSearched <= this->listSize){
			...
			// extend the max & average listsize to the end
			while (last_updated < listsize_collect_sample_points.size()) {
				// check if the max listsize for this run is truly the max across all runs
				this->max_listsize_upto_metric[last_updated] = std::max(this->max_listsize_upto_metric[last_updated], numPathsSearched+1);
			...
			// extend the lower envelop to the end
			while (last_end < sampling_points.size()) {
				lower_envelop[last_end] = running_min_highrate_to_tx;
				last_end++;
			}

			// compare with decoder-wide maximum envelop
			for (size_t i_envelop = 0; i_envelop < this->max_lower_envelop.size(); i_envelop++) {
				this->max_lower_envelop[i_envelop] = std::max(this->max_lower_envelop[i_envelop], lower_envelop[i_envelop]);
			}
...
MessageInformation LowRateListDecoder::genieAided_LowRateDecoding_MaxAngle_ProductMetric_Collector(std::vector<float> receivedMessage, std::vector<int> transmittedCodeword, std::vector<int> punctured_indices, std::vector<float> sampling_points, std::vector<float> listsize_collect_sample_points){
	MessageInformation output;

	// check if angle(Tx, Rx) > theta
	float Tx_to_Rx_angle = utils::compute_angle_between_vectors_rad(receivedMessage, transmittedCodeword, punctured_indices);
	if (Tx_to_Rx_angle >= MAX_ANGLE) {
		// if angle(Tx, Rx) > theta, throw away this run
		output.listSizeExceeded = true;
		std::cerr << "CANNOT BE SAVED.." << std::endl;
		return output;
	} else {
		// if angle(Tx, Rx) <= theta, push to angle boundary
		// receivedMessage = push_to_angle_boundary(receivedMessage, transmittedCodeword, punctured_indices, MAX_ANGLE-0.001);
	}
  ...
  size_t last_end = 0;
    ...
    for (size_t id = last_end; id < sampling_points.size(); id++) {
      if (highrate_to_projected_rx >= sampling_points[id]) {
		...
		// compute angle using product metric
...
MessageInformation LowRateListDecoder::genieAided_LowRateDecoding_MaxAngle_ProductMetric(std::vector<float> receivedMessage, std::vector<int> transmittedCodeword, std::vector<int> punctured_indices, bool push_to_boundary){
	MessageInformation output;
	...
	float Tx_to_Rx_angle = utils::compute_angle_between_vectors_rad(receivedMessage, transmittedCodeword, punctured_indices);
	std::cout << "Tx_to_Rx_angle: " << std::setprecision(4) << Tx_to_Rx_angle << std::endl;
