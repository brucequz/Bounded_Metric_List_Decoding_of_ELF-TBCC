...
LowRateListDecoder::constructLowRateTrellis_ZT(std::vector<float> receivedMessage,
      ...
      for (int forwardPathIndex = 0; forwardPathIndex < numForwardPaths; forwardPathIndex++) {
        ...
        for (int i = 0; i < lowrate_symbolLength; i++) {
          if (metric_type == METRIC_TYPE::PRODUCT_METRIC) {
            branchMetric +=
                -1 * (receivedMessage[lowrate_symbolLength * stage + i] * (float) output_point[i]);
  ...
  for (int stage = lowrate_pathLength - V - 1; stage < lowrate_pathLength - 1; stage++) {
    for (int currentState = 0; currentState < lowrate_numStates; currentState++) {
      ...
      // dealing with cases of uninitialized states, when the transition becomes the optimal father
      // state, and suboptimal father state, in order
      if (!trellisInfo[nextState][stage + 1].init) {
        trellisInfo[nextState][stage + 1].pathMetric = totalPathMetric;
        trellisInfo[nextState][stage + 1].optimalFatherState = currentState;
        trellisInfo[nextState][stage + 1].init = true;
...
MessageInformation
LowRateListDecoder::lowRateDecoding_MaxListsize_ZT(std::vector<float>& receivedMessage) {
  std::vector<std::vector<cell>> trellisInfo;
  ...
  while (numPathsSearched < MAX_LISTSIZE) {
    ...
    if (path[0] == path[lowrate_pathLength - 1] && path[0] == 0 &&
        crc::crc_check(message, crcLength, crc)) {
...
MessageInformation
LowRateListDecoder::lowRateDecoding_MaxAngle_ProductMetric_ZT(std::vector<float> receivedMessage) {
  std::vector<std::vector<cell>> trellisInfo;
  ...
  while (currentAngleExplored < MAX_ANGLE) {
    ...
    // one trellis decoding requires both a tb and crc check
    if (path[0] == path[lowrate_pathLength - 1] && path[0] == 0 &&
        crc::crc_check(message, crcLength, crc) && currentAngleExplored <= MAX_ANGLE) {
      output.message = message;
      output.path = path;
      output.listSize = numPathsSearched + 1;
      output.metric = forwardPartialPathMetric;
...
MessageInformation LowRateListDecoder::lowRateDecoding_MaxMetric_EuclideanMetric_ZT(
  ...
  while (currentMetricExplored < MAX_METRIC) {
    ...
    // one trellis decoding requires both a tb and crc check
    if (path[0] == path[lowrate_pathLength - 1] && path[0] == 0 &&
        crc::crc_check(message, crcLength, crc) && currentMetricExplored <= MAX_METRIC) {
      output.message = message;
      output.path = path;
      output.listSize = numPathsSearched + 1;
      output.metric = forwardPartialPathMetric;
