...
void LowRateListDecoder::generateNeighborList_sequential(const std::vector<float>& allZerosMessage,
  ...
  while (numPathsSearched < this->listSize) {
    ...
    for (size_t index = 0; index < PUNCTURING_INDICES.size(); index++) { codeword[PUNCTURING_INDICES[index]] = 0; }

    // print_int_vector(codeword);
    int metric = 0;
    for (size_t i = 0; i < codeword.size(); i++) {
      if (codeword[i] != 0) metric += std::pow((codeword[i] - allZerosMessage[i]), 2);
    }
    ...
    if (metric > cur_distance) {
      ...
      std::cout << "current distance: " << cur_distance << ", num path (L_d): " << cur_paths << std::endl;
      // write to text file
    ...
    if (metric > thre) {
      ...
      std::cout << "found all neighbors up to distance " << (int)thre << ", total num paths: " << numPathsSearched
                << std::endl;
      return;
    }
    ...
    neighbor_msg[neighbor_idx].insert(neighbor_msg[neighbor_idx].end(), message.begin(), message.end());
    neighbor_cwd[neighbor_idx].insert(neighbor_cwd[neighbor_idx].end(), codeword.begin(), codeword.end());
    ...
    numPathsSearched++;
    if (numPathsSearched % 100000 == 0) { std::cout << "cur path: " << numPathsSearched << std::endl; }
  }
  output.listSizeExceeded = true;
  return;
}

void LowRateListDecoder::generateNeighborList_sequential_TBonly(const std::vector<float>& allZerosMessage,
                                                                double thre) {
  // trellisInfo is indexed [state][stage]
  std::vector<std::vector<cell>> trellisInfo;
  ...
  while (TBPathsSearched < this->listSize) {
    ...
    if (path[0] == path[lowrate_pathLength - 1]) {
      ...
      std::cout << "TB codeword " << TBPathsSearched << ", path[0] = " << path[0] << ", metric = " << metric
                << std::endl;
      ...
      if (metric > cur_distance) {
        ...
        std::cout << "current distance: " << cur_distance << ", num TB offsets: " << cur_paths << std::endl;
      ...
      if (metric > thre) {
        output.listSizeExceeded = false;
        std::cout << "found all neighbors up to distance " << (int)thre << ", total TB paths: " << TBPathsSearched
                  << std::endl;
        return;
      }
...
//         neighborSpectra << "Neighbors of Hamming Weight " << last_curr_metric << ": " << neighbors << std::endl;
//         neighborSpectra << "Non-Neighbors of Hamming Weight " << last_curr_metric << ": " << non_neighbors << std::endl;
...
//       neighbor_messages.push_back(message);
//       neighbor_codewords.push_back(ookcw);
//       if (is_neighbor == 0) { non_neighbors++; }
//     }
...
void LowRateListDecoder::writeDataToFile(const std::vector<std::vector<int>>& data, const std::string& filename) {
  ...
  for (const auto& row : data) {
    ...
    for (size_t i = 0; i < row.size(); ++i) {
      outFile << row[i];
      if (i < row.size() - 1) outFile << " "; // separate values with space
    }
    outFile << "\n";
  }
