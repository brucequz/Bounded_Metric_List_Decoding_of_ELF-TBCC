...
void FeedForwardTrellis::computeGeneratorMatrix() {
  /*
  creates the generator matrix. Each row is the codeword corresponds to a message with exactly one
  1's. The ith row corresponds to a message with the ith position being a 1.
  */

  for (int i = 0; i < Kconv; i++) {
    std::vector<int> m(Kconv, 0);
    m[i] = 1;
    this->generatorMatrix.push_back(this->encode(m));
    utils::print_int_vector(this->encode(m));
  }
}
...
std::vector<int> FeedForwardTrellis::encode(const std::vector<int>& originalMessage) const {
