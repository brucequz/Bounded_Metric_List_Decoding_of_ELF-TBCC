...
float compute_euclidean_distance(const std::vector<T1>& v1,
  ...
  for (size_t i = 0; i < v1.size(); i++) {
    ...
    if (std::find(punctured_indices.begin(), punctured_indices.end(), i) != punctured_indices.end()) { continue; }
...
template <typename T1> float compute_vector_energy(std::vector<T1> vector) {
  if (vector.size() == 0) std::cerr << "EMPTY VECTOR!" << std::endl;
  float sum_of_squares = 0.0;
  for (size_t i = 0; i < vector.size(); i++) { sum_of_squares += float(vector[i] * vector[i]); }
  return sum_of_squares;
}

// computes angle (radians) between two vectors
template <typename T1, typename T2>
float compute_angle_between_vectors_rad(const std::vector<T1>& vec1,
  ...
  if (vec1.size() != vec2.size()) {
    std::cerr << "INVALID INNER PRODUCT DUE TO UNCOMPATIBLE SHAPE! ABORT!" << std::endl;
    exit(1);
  }
...
// Euclidean distance metric
template <typename T1, typename T2>
float sum_of_squares(const std::vector<T1>& v1, const std::vector<T2>& v2, const std::vector<int>& punctured_indices) {
...
template <typename T1, typename T2>
std::vector<float> elementwise_squared_distance(const std::vector<T1>& v1,
                                                const std::vector<T2>& v2,
                                                const std::vector<int>& punctured_indices) {
  if (v1.size() != v2.size()) { throw std::invalid_argument("Vectors must be of the same size"); }
